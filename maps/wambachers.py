...
class Wambachers:
    ...
    def fetch_items_list(self) -> List[WambachersNode]:
        ...
        response = requests.post('https://wambachers-osm.website/boundaries/getJsTree7', params=params, headers=headers)
        return [WambachersNode(id=item['id'], text=item['text'], level=item['data']['admin_level']) for item in response.json()]
    ...
    def fetch_geojson(self, osm_id, level):
        ...
        with open(self.geojson_path, 'wb') as c:
